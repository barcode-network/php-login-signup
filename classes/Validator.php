...
class Validator
{
    ...
    protected function isIdValid(string $id) : bool
    {
        ...
        if (empty($id)) {
            $this->setErrors('natl_id', 'No value given for the national id');
            return false;
        }
        ...
        if (strstr($id, '-')) {
            ...
            $this->setErrors('natl_id', 'Hyphen required for the national id number');
            return false;
        }
        
        if (empty($first_six) || empty($last_four)) {
            $this->setErrors('natl_id', 'Format invalid - part of the id is missing');
            return false;
        }

        if (!ctype_digit($first_six) || !ctype_digit($last_four)) {
            $this->setErrors('natl_id', 'Format invalid - part of the id does not contain numbers');
            return false;
        }

        if (strlen($first_six) <> 6 || strlen($last_four) <> 4) {
            $this->setErrors('natl_id', 'Format invalid - part(s) of the id has an incorrect number of digits.');
            return false;
        }
        return true;
    }
    ...
    {
        if (empty($licenseno)) {
            $this->setErrors('license_num', 'Invalid license number. Cannot be empty');
            return false;
        }
        ...
        if ( $years < 16) {
            $this->setErrors('license_num', 'License number less than 16 years ago ' . $years);
            return false;
        }
        return true;
}   

    protected function isTelephoneValid(array $tel)
    {
        return true;
    }

    protected function isPasswordValid(string $password)
    {
        if (empty($password)) {
            $this->setErrors('user_password', 'No value given for the password');
            return false;
        }else{
            if(strlen($password) > 4){
                return true;
            }else{
                $this->setErrors('user_password', 'Password must be longer than 4 characters');
                return false;
            }
        }
    ...
    protected function isAddressValid(array $address)
    {
        return true;
    }
